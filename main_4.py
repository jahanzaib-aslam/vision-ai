...
def entry_camera(camera_path, yolo_model_name, similarity_threshold, features_his, features_history_lock, global_track_count, global_track_count_lock, check_out_id, check_out_id_lock, model_path, entry_area, frame_size):
    ...
    while cap.isOpened():
        ...
        if len(boxes) != 0:
            for box in boxes:
                ...
                if entry_area and cv2.pointPolygonTest(np.array(entry_area, np.int32), (person_cx, person_cy), False) >= 0:
                ...
                if pose_track_id not in track_history and conf > 0.6:
                # if pose_track_id not in track_history:
                    best_match_id = None
                    best_similarity = 0
                    ...
                    with features_history_lock:         
                        ...
                        if best_match_id:
                            ...
                            if features_his[best_match_id].size(0) > 3:
                                features_his[best_match_id] = features_his[best_match_id][-3:]
                ...
                if pose_track_id in track_history:
                    ...
                    if conf > 0.7:
                        with features_history_lock:
                            ...
                            if features_his[id].size(0) > 3:
                                features_his[id] = features_his[id][-3:]
...
def exit_camera(camera_path, yolo_model_name, similarity_threshold, features_his, features_history_lock, global_track_count, global_track_count_lock, check_out_id, check_out_id_lock, model_path, exit_area, frame_size):
...
def process_camera(camera_path, camera_id, yolo_model_name, similarity_threshold, features_his, features_history_lock, weight_data_dict, weight_data_dict_lock, check_out_id, check_out_id_lock, model_path, frame_size):
...
def handle_weight_changes(client_socket, weight_data_dict, weight_data_dict_lock, GET_WEIGHT, TARE):
...
if __name__ == "__main__":
    ...
    entry_area = [(22, 19),(258, -1),(306, 451),(130, 523)]

    ############################### Yolo & reid model & Similarity Threshold ###############################
    yolo_model_name = "yolov8m-pose.engine"
    model_path = r'C:\Users\mmoht\Downloads\repvgg_a0_person_reid_512 (1)\repvgg_a0_person_reid_512.onnx'
    ...
    # exit_cam = "rtsp://dev:Admin@1234@192.168.1.170:554/cam/realmonitor?channel=4&subtype=0"
    ...
    entry_process = multiprocessing.Process(target=entry_camera, args=(entry_cam, yolo_model_name, similarity_threshold, features_his, features_history_lock, global_track_count, global_track_count_lock, check_out_id, check_out_id_lock, model_path, entry_area, frame_size,))
    
    exit_process = multiprocessing.Process(target=exit_camera, args=(process_cam7, yolo_model_name, similarity_threshold, features_his, features_history_lock, global_track_count, global_track_count_lock, check_out_id, check_out_id_lock, model_path, exit_area, frame_size,))

    # process_process = multiprocessing.Process(target=process_camera, args=(process_cam7, 7, yolo_model_name, similarity_threshold, features_his, features_history_lock, weight_data_dict, weight_data_dict_lock, check_out_id, check_out_id_lock, model_path, frame_size,))
    
    process_process_2 = multiprocessing.Process(target=process_camera, args=(process_cam3, 3, yolo_model_name, similarity_threshold, features_his, features_history_lock, weight_data_dict, weight_data_dict_lock, check_out_id, check_out_id_lock, model_path, frame_size,))

    fetching_product_data_process = multiprocessing.Process(target=fetching_product_data)
    
    # weight_process = multiprocessing.Process(target=handle_weight_changes, args=(client_socket, weight_data_dict, weight_data_dict_lock, GET_WEIGHT, TARE,))

    decission_process = multiprocessing.Process(target=decission_maker)
