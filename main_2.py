...
def entry_camera(camera_path, yolo_model_name, entry_area, exit_area, global_rack_shelves, frame_size):
    ...
    while cap.isOpened():
        ...
        if detection_status_check == True:
            ...
            for box, keypoint in zip(pose_boxes, pose_keypoints):
                ...
                if cv2.pointPolygonTest(np.array(exit_area, np.int32), (person_cx, person_cy), False) >= 0 and track_id in list(track_history.keys()):
                ...
                with weight_data_dict_lock:
                    if len(weight_data_dict) != 0 and action_time not in weight_data_dict:
                        ...
                        if distance_1 <= 110 or distance_2 <= 110:
                            main_session_id = track_history[track_id]

                        # distance_1 = calculate_distance((person_cx, person_cy), global_shelf_location)
                        # if distance_1 <= 120:
                        #     main_session_id = track_history[track_id]



                if track_id in list(track_history.keys()):
...
entry_thread = threading.Thread(target=entry_camera, args=(entry_cam, yolo_model_name, entry_area, exit_area, global_rack_shelves, frame_size,))
